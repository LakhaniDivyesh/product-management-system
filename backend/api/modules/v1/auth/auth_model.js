        ...
        conn.query(login, condition, function (error, result) {
            if (error) {
                ...
                if (result.length > 0) {
                    ...
                    conn.query(update_token, function (error, result) {
                        if (!error && result.affectedRows > 0) {
                            callback('1', { keyword: 'successfully_login', content: {} }, [{ "token": token }]);
                        } else {
                            callback('0', { keyword: 'sql_error', content: { error: "login" } }, error);
                        }
                    })
            ...
            if (response) {
                ...
                conn.query(signup, queryData, function (error, userData) {
                    if (error) {
                        ...
                        var user = `Select * from tbl_users where id = ${userData.insertId} and is_active = 1 and is_deleted = 0;`
